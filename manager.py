...
if not st.session_state["logged_in"]:
    ...
    # --- 2. INTEL TRACKER (AVEC AJOUT MANUEL) ---
    ...
    # --- 3. MATCH ARCHIVE (IMAGE SUR LA MÊME LIGNE) ---
    ...
    # --- 6. STRATÉGIE ---
        ...
        if st.session_state['selected_strat_map'] is None:
            ...
            if deploy_mode:
                ...
                uploaded_file = st.file_uploader("Upload screenshot Valoplant", type=['png', 'jpg', 'jpeg'])
                if uploaded_file is not None:
                    img = Image.open(uploaded_file)
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    img.save(f"{map_folder}/strat_{timestamp}.png")
                ...
                if strats:
                    ...
                    for idx, s in enumerate(reversed(strats)):
                        with cols_img[idx % 3]:
                            st.image(f"{map_folder}/{s}", use_container_width=True)
            else:
                st.markdown(
